...
<div class="bg-white shadow-md rounded-md px-5 pt-3 pb-20 relative min-h-[calc(100vh-145px)]">
    <div class="flex justify-end">
        <a class="bg-[#292c45] text-white px-2 py-2 rounded-md my-5" href="{{ route('admin.themtag') }}">Thêm Tags</a>
    </div>
    <table class="w-full">
        <thead>
        ...
        <tbody class="cursor-pointer">
            ...
            <tr class="hover:bg-yellow-100">
                <td class="border border-gray-300 py-3 text-center">{{($tags->currentPage() - 1) * $tags->perPage() + $key+1}}</td>
                <td class="border border-gray-300 px-2">{{$tag->name}}</td>
                <td class="border border-gray-300 px-2">{{$tag->created_at}}</td>
                <td class="border border-gray-300 px-2">{{$tag->updated_at}}</td>
                <td class="border border-gray-300 px-2">
                    <div class="flex gap-2 justify-center items-center">
                        <a href="{{ route('admin.suaTag', $tag->id) }}">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                            </svg>
                        </a>
                        <form action="/delete_tag/{{$tag->id}}" method="POST" class="flex items-center">
                            @csrf
                            @method('delete')     
